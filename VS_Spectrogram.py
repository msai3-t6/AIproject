...
# etc 라벨 데이터의 평균 Spectrogram 시각화
plt.subplot(3, 1, 1)
librosa.display.specshow(etc_mean, x_axis='time', y_axis='log')
plt.colorbar(format='%+2.0f dB')
plt.title('etc average')

# gaesaekki 라벨 데이터의 평균 Spectrogram 시각화
plt.subplot(3, 1, 2)
librosa.display.specshow(gaesaekki_mean, x_axis='time', y_axis='log')
plt.colorbar(format='%+2.0f dB')
plt.title('gaesaekki average')

# shibal 라벨 데이터의 평균 Spectrogram 시각화
plt.subplot(3, 1, 3)
librosa.display.specshow(shibal_mean, x_axis='time', y_axis='log')
plt.colorbar(format='%+2.0f dB')
