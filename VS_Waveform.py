...
# etc 라벨 데이터 시각화
plt.subplot(3, 1, 1)
librosa.display.waveshow(etc_data, sr=44100)
plt.title('etc')

# gaesaekki 라벨 데이터 시각화
plt.subplot(3, 1, 2)
librosa.display.waveshow(gaesaekki_data, sr=44100)
plt.title('gaesaekki')

# shibal 라벨 데이터 시각화
plt.subplot(3, 1, 3)
librosa.display.waveshow(shibal_data, sr=44100)
plt.title('shibal')

plt.tight_layout()
plt.show()

# 라벨별로 Waveform의 평균값 계산
...
# etc 라벨 데이터의 평균 Waveform 시각화
plt.subplot(3, 1, 1)
librosa.display.waveshow(etc_mean, sr=44100)
plt.title('etc average')

# gaesaekki 라벨 데이터의 평균 Waveform 시각화
plt.subplot(3, 1, 2)
librosa.display.waveshow(gaesaekki_mean, sr=44100)
plt.title('gaesaekki average')

# shibal 라벨 데이터의 평균 Waveform 시각화
plt.subplot(3, 1, 3)
librosa.display.waveshow(shibal_mean, sr=44100)
